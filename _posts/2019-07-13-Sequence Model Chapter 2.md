...
消除偏见的方法类似主成分分析PCA，首先要识别偏见的方向，通过一组完全反应该方向偏见的词相减即可得到嵌入空间中的那一维或者多维偏见方向。下一步是中立：将所有定义上与该方向无关的单词投影到偏见方向上，从而消除偏见。需要消除偏见的单词并不多，可以采取一个分类器来筛选出哪些单词需要进行中立操作。最后对另外一些含有性别差别的单词进行均衡操作，从而使词向量的差别仅仅存在在性别方向。  
