...
最后的model这一节，提交的结果是错误的，所以最终得到了80/100，但是我并没有发现自己的代码有什么问题，只不过测试的时候正确率是100%/72%，比正常的结果正确率稍高。我仔细检查了好几遍，又参考了github上其他几位同学的代码，并没有找出什么错误，所以非常疑惑，如果有读者发现错误，欢迎在下方的评论栏中指出，多谢。  

## Heroes of Deep Learning  
这周采访的嘉宾是Pieter Abbeel,这次采访是两位大牛共处一室谈笑风生，而不是像上周一样用skype视频采访的方式，这样看起来更顺畅一点。  
Pieter Abbeel是一名著名的机器人学家，机器学习、深度学习专家。他在深度学习领域所做出的巨大贡献之一是“深度强化学习(deep reinforcement learning)”，这里贴一个复旦大学邱锡鹏教授的《神经网络与深度学习》中的一章[chap-深度强化学习.pdf](https://nndl.github.io/chap-%E6%B7%B1%E5%BA%A6%E5%BC%BA%E5%8C%96%E5%AD%A6%E4%B9%A0.pdf)，以及维基百科链接[Reinforcement learning-Wikipedia](https://en.wikipedia.org/wiki/Reinforcement_learning)。  
Abbeel认为，目前的深度学习中，“表现”的部分已经很大程度上被解决了，比如说如果数据有一种模式，那么可以用一个深层网络来表现它。但是对于如何将其解构成不同的部分还需要进一步研究，此外，如何让机器在一个长时间视野中进行推理也显得尤为重要。Abbeel尝试着退后一步，让机器学习增强学习算法本身，并且是整个增强学习算法，而不是一部分，这样就可以让一个程序来修改另一个增强算法程序，并且不断的改进它，这就相当于将强化学习放在循环内层。吴恩达的总结是：先用监督学习 来模仿人类做的无论什么事情，之后渐渐地用强化学习来让机器从更长时间视野上去思考。  
接下来是Abbeel对深度学习学习者的建议，他认为最好的方式不是上课读书而是亲自动手去实现，想想自己学了这么久的机器学习，还没有接触到Tensorflow或者Pytorch，而是还在接触最基础的数学原理，还是有些惭愧的。第二个建议是关于读博和工作的选择，他认为这取决于能得到的指导的多少，如果能得到更好的指导，能够学习到更多的知识，那么那个选择就是正确的。我非常认同他的观点，不管是读博还是工作，现在就做知识输出还为时尚早，更为重要的是学习更多新的东西。
