...
```
    ...
    <matplotlib.image.AxesImage at 0x7f02a35eac50>
...
```
    ...
    Z's mean = -0.0228493020112
    Z[3,2,1] = [ 0.         -0.03102354 -0.52995452  0.98611224  0.65733641 -0.84239368
...
```
    ...
    dA_mean = -0.897755420654
    dW_mean = -1.17922594362
    db_mean = -3.64197501464
...
```
    ...
    <ipython-input-17-11d33c897a5d> in <module>()
    ...
    <ipython-input-8-a4072527e333> in pool_forward(A_prev, hparameters, mode)
         47                     # Compute the pooling operation on the slice. Use an if statment to differentiate the modes. Use np.max/np.mean.
         48                     if mode == "max":
    ---> 49                         A[i, h, w, c] = np.max(a_prev_slice)
         50                     elif mode == "average":
         51                         A[i, h, w, c] = np.mean(a_prev_slice)
